...
// DB 사용자 생성 / 업데이트
@Slf4j // Simple Logging Facade for java: 로깅 기능을 간편하게 하는 Lombok annotation
@Service
@RequiredArgsConstructor
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    private final UserRepository userRepository;
    private final RoleRepository roleRepository;
    // User 엔티티를 Spring Security 에서 사용하는 UserPrincipal로 벼환하는 매퍼
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // 2. registrationId 로 provider 구분
        ...
        // 5. 프로젝트 서비스에서 사용하는 UserPrincipal 로 래핑
    ...
    public OAuth2UserInfo convertToUserInfo(AuthProvider provider, Map<String, Object> attributes) {
     ...
     *
     * 1) 이미 존재하면: 이름/이메일 등 프로필 정보를 최신상태로 업데이트
    ...
    protected User upsertUser(AuthProvider provider, OAuth2UserInfo userInfo) {
        // 소셜 서비스에서 제공하는 고유 사용자 ID (구글 sub, kakao id등)
        ...
        // - 동일 provider 안에응 providerId가 유니크
        // - 다른 provider는 providerId가 같더라도 구분가능
        return userRepository.findByProviderAndProviderId(provider, providerId)
                ...
                .orElseGet(() -> {
                    // 2) 처음 로그인 하는 유저인 경우 -> 새 User 엔티티 생성
