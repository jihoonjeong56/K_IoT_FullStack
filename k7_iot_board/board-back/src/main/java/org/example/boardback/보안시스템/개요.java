...
*                                       >> 어디는 누구나 들어와도 되고, 어디는 학생만, 어디는 교사만 출입가능
*
*   로그아웃) "학생증 재발급 권한 기록(RefreshToken)" 삭제
*   OAuth2 - "카카오/구글/네이버 등" 이 대신 학생증을 발급해주고, 해당 학생증으로 학교에 출입하는것
*
...
*       - 지금 방식: 서버는 세션 안쓰고, 매 요청마다 JWT 검사
*               > 선도부나 선생님이 기억이 없음 - 학생증(토큰) 자체에 정보가 들어있음
*
...
*           >> 둘 다 JSON 형태로 응답을 반환핟기 때문에 프론트에서 처리하기 쉬움
*
*   2-2 JwtProvider 역할
