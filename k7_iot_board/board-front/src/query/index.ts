...
// - 자동으로 캐싱해주고
// - 로딩/에러 상태도 관리해주고
// - 새로고침 할 필요 없이 자동으로 최신화해주는 데이터 관리 라이브러리
...
// Zustand = 클라이언트 상태관리 라이브러리 ("전역 상태 관리")
...
// 5) 페이지 이동 후 다시 돌아오는 경우 fetch 부터 재시작
//      >> React Query는 다해줌
...
const { data, isLoading, isError, isSuccess, status, error } = useQuery({
  ...
  // : 1분 동아 새로고침/포커스 이동 시에도 refetch 안함
});
console.log(data);
