 ...
 * 로그인 하지 않은 사용자는 Exception401로 던질 예정
 */

// 1. HandlerInterceptor를 구현해야함
@Component // IoC 컨테이너에 이 클래스를 빈 객체로 등록 함 (싱글톤 패턴으로 관리 됨)
public class LoginInterceptor implements HandlerInterceptor {
    /**
     * preHandle : 컨트롤러 진입전에 실행되는 메서드
     ...
     * @return true (컨트롤러로 진입) / false(진입 안됨)
     * @throws Exception
     */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(sessionUser == null){
            throw new Exception401("로그인 안됨");
        }
        // 로그인 정보가 있으면 컨틀롤러로 진입 허용
        return true;
    }
